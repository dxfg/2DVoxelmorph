...
import 2D_revised_networks
import 2D_revised_losses


vol_size = (1024, 1024)
base_data_dir = '/Your directory here/'
...
def train(model,save_name, gpu_id, lr, n_iterations, reg_param, model_save_iter):
    ...
    with tf.device(gpu):
        model = 2D_networks.unet(vol_size, nf_enc, nf_dec)
        model.compile(optimizer=Adam(lr=lr), loss=[
                      2D_losses.cc2D(), 2D_losses.gradientLoss2D()], loss_weights=[1.0, reg_param])
        # model.load_weights('../models/udrnet2/udrnet1_1/120000.h5')

    train_example_gen = 2D_datagenerators.example_gen(train_vol_names)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_name", type=str,
                        dest="save_name", default= 'machine_testing')
    parser.add_argument("--gpu", type=int,default=0,
                        dest="gpu_id", help="gpu id number")
    parser.add_argument("--lr", type=float, 
                        dest="lr", default=1e-4,help="learning rate") 
    parser.add_argument("--iters", type=int, 
                        dest="n_iterations", default=150,
